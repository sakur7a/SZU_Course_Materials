...
class MobileNetV2HeadDropout(nn.Cell):
    """在特征向量上做 Dropout + 线性分类。

    说明：MindSpore 的 nn.Dropout 参数是 keep_prob（保留概率），不是丢弃概率。
    """

    def __init__(self, input_channel=1280, num_classes=1000, keep_prob=0.8):
        super(MobileNetV2HeadDropout, self).__init__()
...
class MobileNetV2HeadMLP(nn.Cell):
    """两层 MLP head：Dense -> ReLU -> Dropout -> Dense。

    用于在冻结 backbone 的前提下增加 head 表达能力。
    """

    def __init__(self, input_channel=1280, hidden_dim=256, num_classes=1000, keep_prob=0.8):
        super(MobileNetV2HeadMLP, self).__init__()
        self.head = nn.SequentialCell([
            nn.Dense(input_channel, hidden_dim, has_bias=True),
            nn.ReLU(),
            nn.Dropout(keep_prob),
            nn.Dense(hidden_dim, num_classes, has_bias=True),
        ])
        self._initialize_weights()
